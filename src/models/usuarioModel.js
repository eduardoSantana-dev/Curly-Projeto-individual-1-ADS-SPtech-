...
function login(email, senha) {
  let query = `select idUsuario as id,nome,arroba,img,curvatura,email from usuario where (email = '${email}' or arroba = '${email}') and senha ='${senha}' and statusUsuario = 'ativo';`;
  return database.executar(query);
}
...
left join seguir_usuario espectador on u.idUsuario = espectador.idUsuarioSeguido and espectador.idUsuarioSeguidor = ${idEspectador}
where idUsuario = ${id};`;
  return database.executar(query);
}
