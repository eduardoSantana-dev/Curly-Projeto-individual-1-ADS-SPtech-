...
async function verificarSenha(req, res) {
  try {
    const resposta = await usuarioModel.verificarSenha(
      req.body.idUser,
      req.body.senha,
    );
    console.log(resposta);
...
async function atualizarUser(req, res) {
  ...
  try {
    const resposta = await usuarioModel.atualizarUser(
      user.id,
      user.nome,
      user.arroba,
      user.email,
...
async function buscarPopulares(req, res) {
  try {
    let resposta = await usuarioModel.buscarPopulares();
    res.status(202).json({
      sucess: true,
      data: resposta,
    });
...
async function buscarSeguindo(req, res) {
  try {
    let resposta = await usuarioModel.buscarSeguindo(req.params.idUser);
    res.status(202).json({
      sucess: true,
      data: resposta,
    });
